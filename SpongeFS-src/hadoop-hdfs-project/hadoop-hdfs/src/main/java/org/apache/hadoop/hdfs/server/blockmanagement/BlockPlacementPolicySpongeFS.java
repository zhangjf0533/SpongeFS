...
public class BlockPlacementPolicySpongeFS extends BlockPlacementPolicy {
  ...
  private static final ThreadLocal<HashMap<org.apache.hadoop.hdfs.server.blockmanagement.BlockPlacementPolicySpongeFS.NodeNotChosenReason, Integer>>
          CHOOSE_RANDOM_REASONS = ThreadLocal
          .withInitial(() -> new HashMap<org.apache.hadoop.hdfs.server.blockmanagement.BlockPlacementPolicySpongeFS.NodeNotChosenReason, Integer>());
  ...
  protected DatanodeStorageInfo chooseRandom(int numOfReplicas,
      ...
      if (clusterMap instanceof DFSNetworkTopology) {
        for (StorageType type : storageTypes.keySet()) {
          ...
          logNodeIsNotChosen(null,
                  org.apache.hadoop.hdfs.server.blockmanagement.BlockPlacementPolicySpongeFS.NodeNotChosenReason.NO_REQUIRED_STORAGE_TYPE,
    ...
    if (numOfReplicas>0) {
      ...
      final HashMap<org.apache.hadoop.hdfs.server.blockmanagement.BlockPlacementPolicySpongeFS.NodeNotChosenReason, Integer> reasonMap =
  ...
  DatanodeStorageInfo chooseStorage4Block(DatanodeDescriptor dnd,
    ...
    if (storage != null) {
      ...
      logNodeIsNotChosen(dnd, org.apache.hadoop.hdfs.server.blockmanagement.BlockPlacementPolicySpongeFS.NodeNotChosenReason.NOT_ENOUGH_STORAGE_SPACE,
  ...
  private static void logNodeIsNotChosen(DatanodeDescriptor node,
                                         org.apache.hadoop.hdfs.server.blockmanagement.BlockPlacementPolicySpongeFS.NodeNotChosenReason reason) {
    logNodeIsNotChosen(node, reason, null);
  }

  private static void logNodeIsNotChosen(DatanodeDescriptor node,
                                         org.apache.hadoop.hdfs.server.blockmanagement.BlockPlacementPolicySpongeFS.NodeNotChosenReason reason, String reasonDetails) {
    assert reason != null;
    if (LOG.isDebugEnabled()) {
  ...
  boolean excludeNodeByLoad(DatanodeDescriptor node){
    ...
    if ((nodeLoad > maxLoad) && (maxLoad > 0)) {
      logNodeIsNotChosen(node, org.apache.hadoop.hdfs.server.blockmanagement.BlockPlacementPolicySpongeFS.NodeNotChosenReason.NODE_TOO_BUSY,
  ...
  boolean isGoodDatanode(DatanodeDescriptor node,
    ...
    if (!node.isInService()) {
      logNodeIsNotChosen(node, org.apache.hadoop.hdfs.server.blockmanagement.BlockPlacementPolicySpongeFS.NodeNotChosenReason.NOT_IN_SERVICE);
      return false;
    }

    if (avoidStaleNodes) {
      if (node.isStale(this.staleInterval)) {
        logNodeIsNotChosen(node, org.apache.hadoop.hdfs.server.blockmanagement.BlockPlacementPolicySpongeFS.NodeNotChosenReason.NODE_STALE);
        return false;
      }
    }
    ...
    if (counter > maxTargetPerRack) {
      logNodeIsNotChosen(node, org.apache.hadoop.hdfs.server.blockmanagement.BlockPlacementPolicySpongeFS.NodeNotChosenReason.TOO_MANY_NODES_ON_RACK);
      return false;
    }
    ...
    if (dataNodePeerStatsEnabled && excludeSlowNodesEnabled) {
      ...
      if (slowNodesUuidSet.contains(node.getDatanodeUuid())) {
        logNodeIsNotChosen(node, org.apache.hadoop.hdfs.server.blockmanagement.BlockPlacementPolicySpongeFS.NodeNotChosenReason.NODE_SLOW);
        return false;
      }
    }
