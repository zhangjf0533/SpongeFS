...
public class BlockPlacementPolicyRabbit extends BlockPlacementPolicy {
  ...
  private static final ThreadLocal<HashMap<org.apache.hadoop.hdfs.server.blockmanagement.BlockPlacementPolicyRabbit.NodeNotChosenReason, Integer>>
          CHOOSE_RANDOM_REASONS = ThreadLocal
          .withInitial(() -> new HashMap<org.apache.hadoop.hdfs.server.blockmanagement.BlockPlacementPolicyRabbit.NodeNotChosenReason, Integer>());
  ...
  protected BlockPlacementPolicyRabbit() {
  }
  ...
  private void sortNodesByIP(List<Node> nodes) {
  ...
  public DatanodeStorageInfo[] chooseTarget(String srcPath,
    ...
    for (Node node : nodes) {
      if (node instanceof DatanodeDescriptor) {
        ...
        for (DatanodeStorageInfo storage : dn.getStorageInfos()) {
          int nums = storage.numBlocks();
          bm += nums;
          blocksPerNode.add(nums);
        }
      }
    }
    ...
    for (int i = p; i < n; i++) {
      ...
      double difference = bm *1.0 / (i + 1) - currentBlocks;
  ...
  protected DatanodeStorageInfo chooseRandom(int numOfReplicas,
      ...
      if (clusterMap instanceof DFSNetworkTopology) {
        for (StorageType type : storageTypes.keySet()) {
          ...
          logNodeIsNotChosen(null,
                  org.apache.hadoop.hdfs.server.blockmanagement.BlockPlacementPolicyRabbit.NodeNotChosenReason.NO_REQUIRED_STORAGE_TYPE,
    ...
    if (numOfReplicas>0) {
      ...
      final HashMap<org.apache.hadoop.hdfs.server.blockmanagement.BlockPlacementPolicyRabbit.NodeNotChosenReason, Integer> reasonMap =
  ...
  DatanodeStorageInfo chooseStorage4Block(DatanodeDescriptor dnd,
    ...
    if (storage != null) {
      ...
      logNodeIsNotChosen(dnd, org.apache.hadoop.hdfs.server.blockmanagement.BlockPlacementPolicyRabbit.NodeNotChosenReason.NOT_ENOUGH_STORAGE_SPACE,
  ...
  private static void logNodeIsNotChosen(DatanodeDescriptor node,
                                         org.apache.hadoop.hdfs.server.blockmanagement.BlockPlacementPolicyRabbit.NodeNotChosenReason reason) {
    logNodeIsNotChosen(node, reason, null);
  }

  private static void logNodeIsNotChosen(DatanodeDescriptor node,
                                         org.apache.hadoop.hdfs.server.blockmanagement.BlockPlacementPolicyRabbit.NodeNotChosenReason reason, String reasonDetails) {
    assert reason != null;
    if (LOG.isDebugEnabled()) {
  ...
  boolean excludeNodeByLoad(DatanodeDescriptor node){
    ...
    if ((nodeLoad > maxLoad) && (maxLoad > 0)) {
      logNodeIsNotChosen(node, org.apache.hadoop.hdfs.server.blockmanagement.BlockPlacementPolicyRabbit.NodeNotChosenReason.NODE_TOO_BUSY,
  ...
  boolean isGoodDatanode(DatanodeDescriptor node,
    ...
    if (!node.isInService()) {
      logNodeIsNotChosen(node, org.apache.hadoop.hdfs.server.blockmanagement.BlockPlacementPolicyRabbit.NodeNotChosenReason.NOT_IN_SERVICE);
      return false;
    }

    if (avoidStaleNodes) {
      if (node.isStale(this.staleInterval)) {
        logNodeIsNotChosen(node, org.apache.hadoop.hdfs.server.blockmanagement.BlockPlacementPolicyRabbit.NodeNotChosenReason.NODE_STALE);
        return false;
      }
    }
    ...
    if (counter > maxTargetPerRack) {
      logNodeIsNotChosen(node, org.apache.hadoop.hdfs.server.blockmanagement.BlockPlacementPolicyRabbit.NodeNotChosenReason.TOO_MANY_NODES_ON_RACK);
      return false;
    }
    ...
    if (dataNodePeerStatsEnabled && excludeSlowNodesEnabled) {
      ...
      if (slowNodesUuidSet.contains(node.getDatanodeUuid())) {
        logNodeIsNotChosen(node, org.apache.hadoop.hdfs.server.blockmanagement.BlockPlacementPolicyRabbit.NodeNotChosenReason.NODE_SLOW);
        return false;
      }
    }
